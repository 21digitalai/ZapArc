...
import { extractLnurlPaymentComment, loadPaymentComment, renderTransactionCommentDetailRow } from './payment-comment';
...
function showTransactionDetail(tx: StoredTransaction): void {
    ...
    detailRows += renderTransactionCommentDetailRow(tx.description, tx.comment, escapeHtml);
...
async function executeSettingsChangePin(): Promise<void> {
...
const runSettingsPinChange = createSingleFlight(executeSettingsChangePin);

async function handleSettingsChangePin(): Promise<void> {
    await runSettingsPinChange();
}
